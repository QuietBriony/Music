...
assert.equal(windowMock.BandRoomTestHooks?.BANDROOM_APP_VERSION, "br-184-vocal-pocket", "Band Room should expose the current vocal-pocket version");
...
assert.equal(migratedMixPrefs.mixPrefsVersion, "v301-human-fly-body", "Migrated prefs should record current mix version");
...
assert.match(html, /band-room\.js\?v=br-184/, "Band Room HTML should reference the current JS cache marker");
...
assert.match(sw, /band-room\.js\?v=br-184/, "Service worker should precache the current Band Room JS marker");
...
assert.match(source, /stemBus\.vocals = new Tone\.Gain\(0\.58\)/, "Vocal stem should sit in the wall/pocket (v311 0.60→0.58, dry path up instead)");
...
assert.match(source, /const MIX_PREFS_VERSION = "v301-human-fly-body"/, "Band Room should version saved mix defaults");
