...
for (const required of [
  ...
  "band-room.html?band=hazama&aiLight=1",
  "lyric-lab.html",
...
for (const marker of [/HAZAMA/, /v388/, /v390/, /60[–-]90秒/, /6分/, /phone-light/, /2音サイレン/, /synth-only/, /START/, /lyric-lab\.html/]) {
...
const feedbackCurrentStart = feedback.indexOf("### 2026-08-02 HAZAMA v388/v390 current pass");
...
assert.match(feedbackCurrent, /v388[\s\S]*v390/, "Listening backlog current note should cover v388 and v390");
assert.match(feedbackCurrent, /60[–-]90 seconds[\s\S]*six-minute/i, "Listening backlog should separate quick and long passes");
...
assert.match(ls03[0], /v388\/v390[\s\S]*BL-041/, "LS-03 should separate static QA from BL-041 listening");
