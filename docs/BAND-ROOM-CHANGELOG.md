...
## v260 compact — AI 再現の**デフォルト整合性**と**動線**を整える（監査パス）
...
を全 sweep。発見した「v259 までの蓄積で生じた矛盾／UX 不整合」を v260
で吸収。

### v260 の修正（band-room.js / bands.json / scripts/check-band-room-logic.mjs）
...
原音不変。CPU 影響なし。プレフ migration 追加なし（v260 は default 値の
swap だけで saved state を触らない）。

- `band-room.js?v=br-146`、`hazama-fm-v260`。
