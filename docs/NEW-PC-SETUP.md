...
| Codex App / Codex CLI | 別 PC での実装・レビュー・project 表示確認 |
...
2. C:\workspace\music-stack\Music\docs\PC-REGISTRY.md
   (自分の machineName = "studioPC" の役割節を熟読 — UR44 試聴 + DAW、
...
- SESSION-LEDGER 追記は ## YYYY-MM-DD [studioPC] — ... 形式
...
- studioPC の役割範囲 (ear-verified iteration) で着手するのに適した
...
最初に「OK、studioPC セットアップを開始します」と返事してから段階
1 を実行。
```

`<PC名>` と `'studioPC'` の部分を、別 PC で使う時はそれぞれ該当の名前
...
## Codex App project 表示の再現

Codex App のチャット履歴は repo や別 PC に自動同期されない。別 PC で同じ
project context を出す正攻法は、同じパスに clone して、Codex App から
`C:\workspace\music-stack\Music` を project / folder として開き、repo 内の
`AGENTS.md` と docs から context を復元すること。

別 PC での手順:

1. 上記手順で 5 repo を `C:\workspace\music-stack\<repo>` に clone。
2. Codex App で `C:\workspace\music-stack\Music` を開く。
3. 初回に `AGENTS.md`、`docs/autonomy/STACK-INDEX.md`、
   `docs/autonomy/SESSION-LEDGER.md` を読む。
4. `git rev-parse --show-toplevel` が `C:/workspace/music-stack/Music` を返すか確認。
5. sibling repo を触る時だけ、その repo の `AGENTS.md` を読む。

`C:\workspace\music-stack` 直下から Codex App を開いた場合、そのフォルダは
Git repo ではなく workspace container。project 表示の案内用に、必要なら
その PC ローカルで以下を `C:\workspace\music-stack\AGENTS.md` として置く。
このファイルは親フォルダが Git 管理外なので、repo には入らない。

```md
# AGENTS.md - music-stack workspace map

This directory is a workspace container, not the main Git repository.
...
When a Codex session is started from `C:\workspace\music-stack`, switch to
`C:\workspace\music-stack\Music` for Music repo work. Each sibling repository
has its own `AGENTS.md`; read that file before editing that repository.
```

2026-05-30 にやった復旧内容:

- `Music/AGENTS.md` に `Workspace map` を追加。
- gamingpc の `C:\workspace\music-stack\AGENTS.md` に workspace map をローカル追加。
- 「チャット履歴は別 PC の Codex App へ自動同期されない。repo docs で context を
  復元する」という運用に整理。
