...
### `studioPC` (UR44 PC、Intel Surface)

旧名 / 過去ログ上の prefix は `studio-surface`。新規 setup、local git config、
SESSION-LEDGER 追記では `studioPC` を使う。
...
|---|---|---|
| `engine.js` / `audio/music-*.js` | chouta-surface, studioPC | worker-gaming |
| `fm.html` / `index.html` / `fm.js` / `fm.css` | chouta-surface, studioPC | worker-gaming |
| `band-room.html` / `band-room.js` / `band-room.css` | **全 PC 触らない** (Band Room チャット専用) | (全部) |
| `presets/drum-frames-*.json` | worker-gaming (生成)、chouta-surface (review) | studioPC |
| `references/*.json` | chouta-surface | worker-gaming, studioPC |
| `docs/autonomy/*` | chouta-surface, studioPC (LEDGER 追記のみ) | worker-gaming (autonomy session せず) |
| `docs/HAZAMA-FM-ARCHITECTURE.md` 等 design doc | chouta-surface | studioPC, worker-gaming |
| DAW project files (将来 repo に入れるなら) | studioPC | chouta-surface, worker-gaming |
...
```
Co-Authored-By: Claude Opus 4.7 (studioPC) <noreply@anthropic.com>
```
