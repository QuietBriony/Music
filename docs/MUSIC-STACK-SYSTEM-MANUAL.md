...
| 静かに聴く | [Chill Session](https://quietbriony.github.io/chill/session.html) | `START`、必要なら `BASS` / `AUTO`（数分で景色が移る long-form weather） |
| ドラム不在で叩きたい | [drum-floor](https://quietbriony.github.io/drum-floor/) | `🎤 バンドを聴いて鳴らす`（周りの音を聴いて追従）／ 曲・手動なら `再生（SYNC/手動）` |
| 水・庭の環境を流す | [namima](https://quietbriony.github.io/namima/) | `Tap to start`、あとは自走（潮 tide で使う音と景色が移る） |
| 今の状態を保存する | `SYNC` | 音声ではなく metadata-only の地図を保存 |
| 人間確認を残す | [Review Queue](https://quietbriony.github.io/openclaw/#human-review) | 音量、genre、Desk導線などの確認待ちを見る（Desk に統合済み） |
...
| drum-floor | AI 共演ドラマー / groove | `🎤 バンドを聴いて鳴らす`（mic 追従 co-player）／ 手動 preview / candidate | 自動 arm しない・mic は録音せず特徴量のみ |
| namima | safe ambient / water mood | `Tap to start`、あとは自走（潮 tide）/ mood / trace / ripple | 暗いglitchや低域圧を足さない |
| chill | quiet piano / trio / long-form listening | piano, bass, soft drums（long-form weather で数分かけて景色が移る） | STARTは人間操作 |
...
- [Chill Session](https://quietbriony.github.io/chill/session.html)
- [Human review queue](https://quietbriony.github.io/openclaw/#human-review)（OpenClaw Desk に統合）
