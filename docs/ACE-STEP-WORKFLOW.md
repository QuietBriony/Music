...
## 4.5 ハードウェア現実 — chouta-surface では**ローカル不可**

開発機 chouta-surface は **Snapdragon X Elite (X1E80100) / ARM64 Windows / GPU は
Qualcomm Adreno X1-85 / CUDA なし**（[BL-023](./autonomy/BACKLOG.md) の「ARM 版」）。
ACE-Step は PyTorch ベースで、現実的に動かすには CUDA(NVIDIA) か Apple Silicon(MPS)、
最低でも x86 が要る。chouta-surface では:

- **CUDA なし** → GPU 高速経路が使えない
- **ARM64 Windows の PyTorch が未成熟**（公式 wheel がほぼ無い）→ 依存導入で詰まりやすい
- **Adreno** は PyTorch/CUDA/ROCm の対象外

→ **chouta-surface でローカル生成はしない**。ACE-Step を使うなら下のどちらか:

1. **クラウド/レンタル GPU**（Colab / RunPod / Lambda / HF Spaces 等）で生成し wav を
   手元へ download。ACE-Step の利点（OSS・LoRA・素材を外部学習に出さない選択肢）が要る時。
2. **別マシン**（NVIDIA GPU PC or Apple Silicon Mac）が手元にあればそこで。

**chouta-surface だけで歌入りデモを作るなら ACE-Step ではなく [Suno](./SUNO-WORKFLOW.md)**
（クラウド・ブラウザで動くので ARM Surface でも問題なし）。ローカルで動かせないなら
ACE-Step の「ローカル/無料/LoRA」優位は効かないので、目的（Tabasco デモ→アレンジ指針）
には Suno が手っ取り早い。
