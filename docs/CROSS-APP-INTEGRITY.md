# Cross-App Integrity — Music Stack 全体の整合性チェック (v155 時点)
...
v142-v155 で、FM の phrase-lock / pattern variation / genre handoff / 40Hz focus / 長時間 event quieting / start-stop hardening / bridge health fallback / browser loudness uplift と、Band Room の車載 audio bridge / route status / bridge health fallback / album-flow default playback / lock-screen album transport / drum-floor metadata handoff が追加済み。
...
|-----|---------------------|------------|
| Band Room | `band-room.css?v=br-70` / `band-room.js?v=br-75` | hazama-fm-v155 |
| Hazama FM | `engine.js?v=fm-80` / `fm.css?v=fm-47` / `fm.js?v=fm-60` | 同上 |
...
| doc | カバー範囲 | 整合状態 |
|-----|----------|--------|
| `BAND-ROOM-MANUAL.md` | Band Room 入り口 | v155 主要機能反映済 |
| `BAND-ROOM-CHANGELOG.md` | v65-v155 compact | 近年履歴は compact 追記で同期 |
...
| `CROSS-APP-INTEGRITY.md` | **このファイル** | 整合性 hub / v155 cache map |
