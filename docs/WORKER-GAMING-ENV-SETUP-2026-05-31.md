...
2. Scan VST3 from `C:\Program Files\Common Files\VST3`.
3. Scan NI VST2 from `C:\Program Files\Native Instruments\VSTPlugins 64 bit` only when needed.
4. Export polished audio to `C:\workspace\music-stack-worker\daw-export\tabasco\human-fly`.
5. Review in Band Room before promoting any metadata or candidates into Git.
