...
4. 良い瞬間で `SYNC` を押す。
5. Musicの表示か [Desk / OpenClaw repo hub](https://quietbriony.github.io/openclaw/) で推奨行き先を見る。
6. 行き先を開き、人間が START / 再生 / Tap to start を押す。
...
外部/内部repoのアイデアを拾う時は、OpenClaw Desk の `Repo Harvest` を使います。
ここでいう OpenClaw Desk は LLM処理先ではなく、`QuietBriony/openclaw` repo の Pages ハブです。

1. [Desk / OpenClaw repo hub](https://quietbriony.github.io/openclaw/#repo-harvest) を開く。
...
- `OpenClaw Desk raw candidate`: Surface CLI で MIDI候補を作る別導線。候補生成と inspect までで、arm/録音/upload は人間確認です。
