...
(マナーモードでは音が出ません)。
Hazama FM は起動時にブラウザ側の OUTPUT 88 までフェードインします。
...
| **LOFI** | Salamander piano trio + breakbeat、薄い vinyl crackle / jazz dust | カフェで本を読む時 |
...
`FOCUS 30` / `DRIVE 30` / `NIGHT 30` は 30 分の自動 genre arc です。
作業用なら `FOCUS 30`、移動中なら `DRIVE 30`、寝る前は `NIGHT 30` が基準。

`40HZ` は focus mode です。既定は OFF。ON にすると master bus に
40 Hz / 8% の浅い AM を 3 秒でフェードインします。設定は localStorage に保存。
AI fill burst 中は自動で 0% に退避し、終わると戻ります。

`AI fill` は Magenta DrumsRNN で次の 4 小節だけ drum variation を足します。
通常の engine 側にも 4 小節ごとの小 variation / 16 小節 reset が入っているため、
普段は触らなくても長時間で少しずつ表情が変わります。
...
// 4 bar pattern variation の console log を一時的に出す
localStorage.setItem("music.debug.patternVariation", "1")

// phrase-locked mode change の console log を一時的に出す
localStorage.setItem("music.debug.barCounter", "1")
```
