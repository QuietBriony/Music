...
| **TECHNO** | sparse machine drum + stronger acid pulse | 作業の手を動かす時 |
| **LOFI** | レイジーな pocket + vinyl crackle | カフェで本を読む時 |
| **JAZZ** | brush + walking bass + ride | 夜、文章を書く時 |
| **FUNK** | clavi + EP + 跳ねる pocket | 元気を出したい時 |
| **PIANO** | dry felt-piano anchor + chord bed + memory reply | 静かな深い集中 |
...
別 genre へ移ります。通常の長時間作業 BGM は `shuffle` OFF か `ANY` が基準です。
