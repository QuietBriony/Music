...
python scripts/audit.py   # 現状 0 BAD 確認
```
...
```bash
python scripts/audit.py   # 0 BAD 必須
git status                # 変更ファイル確認
...
5. `python scripts/audit.py` で再検証
...
毎 commit 直前に audit.py を回す習慣で、整合性は automatic に保たれる。
...
| 課題 | 解決 |
|---|---|
| 両 agent が同じ整合性ガードを使う | `python scripts/audit.py` が 1 つの真実 |
