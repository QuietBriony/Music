...
python -X utf8 scripts/audit.py --expected-version hazama-fm-v153   # 現状 0 BAD 確認
...
```bash
python -X utf8 scripts/audit.py --expected-version hazama-fm-v153   # 0 BAD 必須
node scripts/check-js.mjs
node scripts/check-band-room-logic.mjs
git status                # 変更ファイル確認
...
5. `python -X utf8 scripts/audit.py --expected-version hazama-fm-v153` と `node scripts/check-js.mjs` で再検証
...
jobs:
  audit:
    ...
    steps:
      ...
      - run: python -X utf8 scripts/audit.py --expected-version hazama-fm-v153
...
| 課題 | 解決 |
|---|---|
| 両 agent が同じ整合性ガードを使う | `python -X utf8 scripts/audit.py --expected-version hazama-fm-v153` + `node scripts/check-js.mjs` が 1 つの真実 |
