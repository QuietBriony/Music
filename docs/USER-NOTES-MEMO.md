...
5. ✅ **旋律単調さ** — v169 で melodic director を追加。8〜16 bar ごとに key center / contour / chord turn を動かし、固定断片の反復感を減らした。
6. ✅ **ベースライン単調さ** — v170 で bassline director を追加。4〜8 bar ごとに bass gate / interval walk / ghost push を動かし、Hazama FM と Music Core Rig の main synth / lofi / dub bass を共通で改善。
...
- v161 (2026-05-15): Band Room の原音 album flow を full-song duration guard に変更。structure が先に終わっても stem/catalog 尺まで次曲へ進まず、iPhone sleep 復帰時は stuck note を release して Transport / stems / bridge を再同期
- v162 (2026-05-15): Band Room の Tabasco 歌詞候補を `docs/tabasco-lyrics-final.md` の 1 本に統合。footer も `final singable` のみ表示
- v163 (2026-05-15): Band Room の cache-busted lyrics / drum frame fetch を SW 側の `ignoreSearch` fallback で offline-safe 化。`Human Fly` 歌詞を final 一本稿のまま歌いやすく微修正し、Band Room / Drum Floor 導線を手動previewとして見える化
- v164 (2026-05-15): Music Core overview に Chill Session 導線を追加。chill 側も Music SYNC が metadata-only で、音は人間の START まで始まらないことを入口と戻りリンクで明確化
- v165 (2026-05-15): Band Room に普通の分秒 timeline / seek bar を追加。停止中に任意地点へ動かして START、再生中 seek で stems / section / lyrics が同じ位置へ同期
- v166 (2026-05-15): Band Room AI 再現の bass / guitar / vocal / chords を source-derived part agent 化。原曲 drum-frame の accent / section role / chord から各パートが自律 pattern を組み、bass/guitar 初期音色も electric sampler に寄せた
- v169 (2026-05-16): Hazama FM に melodic director を追加。固定的な D/F#/G/E 断片を 8〜16 bar phrase の key/contour/chord turn で動かし、旋律がずっと同じに聞こえる問題を緩和
- v170 (2026-05-16): Hazama FM / Music Core Rig に bassline director を追加。main synth bass の gate/note/duration と lofi Salamander / dub electric bass の walking を phrase-aware にし、固定 root/5th/octave loop の単調さを緩和
- v171 (2026-05-16): Band Room 専用 PWA manifest を追加。iPhone / mobile screen lock の blur / freeze / pagehide 時に sampler も含めた panic release を複数回走らせ、画面を閉じる瞬間の単音 stuck loop を減らす
- v172 (2026-05-16): Hazama FM に groove conversation メタデータを追加
- v173 (2026-05-16): Hazama conversation metadata を stack 全体に同期
- v174 (2026-05-16): music-stack 自律開発エンジンを構築。`docs/autonomy/`（STACK-INDEX / BACKLOG / SESSION-LEDGER / AUTONOMOUS-RUN）、`scripts/stack-check.mjs`、sister repo 4 本の AGENTS.md を整備。runtime / 音の変更なし
- v175 (2026-05-16): 自律開発エンジンを Claude + Codex 共同開発対応に。AUTONOMOUS-RUN に Codex on-ramp と共同開発セクション、BACKLOG に claim ルールと `status` フィールド、CODEX-HANDOFF の workflow を engine 連動へ更新。runtime / 音の変更なし
- v176 (2026-05-16): Hazama FM funk voice の出音改善。funk EP (Rhodes) の和音を一括同時発音から per-note ロール（8-22ms ずらし）+ voice ごとの velocity ばらつきへ、funk clavi のフィルタを velocity 追従に（静的 cutoff で全ノート同一音色だったのを解消）。`audio/genre-flavor.js`、engine.js 凍結のため同ファイルで実装。同 cycle で drum-floor の出音（hit ごとの variation + kick の boom/level 抑制）も改善
