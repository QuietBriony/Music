...
(function () {
  ...
  function scheduleSynthBandTeardown() {
    ...
    synthBandTeardownTimer = setTimeout(() => {
      ...
      // HAZAMA layers (v365): also free the always-on arp LFO/FeedbackDelay +
      // bass Distortion so 原音 isn't left processing standing DSP on masterGain.
      // Null arpLfo/bassSeqFilter too so the section-arc ramps don't touch a
      // disposed node (they're already try/catch-guarded, but keep it clean).
      if (arpSynth === captured.arpSynth) { arpSynth = null; arpLfo = null; }
      if (bassSeqSynth === captured.bassSeqSynth) { bassSeqSynth = null; bassSeqFilter = null; }
    }, 1800);
  }
  ...
  // or ?band=<id> / ?bandId= / ?dev=1, which flips this unlock on.
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    // Hidden-band deep entry: ?band=<id> (or ?bandId=, e.g. the footer "◦ hazama"
    // link) opens a ui_hidden band (HAZAMA WIP) without exposing it in the selector.
    try {
      const p = new URLSearchParams(window.location.search);
